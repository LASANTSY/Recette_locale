...
const Sidebar: React.FC<SidebarProps> = ({ 
  items,
  activeItem: propActiveItem = 'home',
  onItemClick = () => {},
  className = '',
  // accentColor = '#00C21C',
  ...
  // Détermine l'item actif en fonction de l'URL actuelle
  const activeItem = items.find(item => location.pathname.startsWith(item.route))?.id || propActiveItem;
      ...
      >
        ...
        <div className={`flex items-center ${sidebarCollapsed ? 'justify-center p-[18px]' : 'justify-between p-[13px]'} ${isMobile ? 'p-[18px]' : ''} border-b ${theme === 'dark' ? 'border-gray-700' : 'border-gray-300'}`}> 
          ...
          {!isMobile && !sidebarCollapsed && (
            <button
              ...
              className={`hidden md:flex rounded-md transition-colors py-[0.65em] px-[0.8em] cursor-pointer
        ...
        {!isMobile && sidebarCollapsed && (
          <div className="flex justify-center pt-2">
            <button
              onClick={onToggleCollapse}
              className={`rounded-md transition-colors py-[0.7em] px-[0.8em] cursor-pointer
                ${theme === 'dark' ? 'hover:bg-gray-700 text-gray-400 hover:text-gray-200' : 'hover:bg-gray-100 text-gray-500 hover:text-gray-700'}`}
                ...
                <li key={item.id} className="relative">
                  <button
                    ...
                    className={`
                      ...
                      transition-all duration-200 ease-in-out group relative cursor-pointer
                      ${isActive 
                        ? `active font-medium border border-opacity-30 border-[#00C21C] bg-[rgba(0,194,28,0.15)]`
                    ...
                    {sidebarCollapsed && (
                      ...
                      >
                        ...
                        <div className={`absolute top-1/2 -left-1 transform -translate-y-1/2 w-0 h-0 border-t-4 border-b-4 border-r-4 border-transparent
                          ${theme === 'dark' ? 'border-r-gray-800' : 'border-r-gray-800'}`}
                        />
                      </div>
                    )}
                  </button>
